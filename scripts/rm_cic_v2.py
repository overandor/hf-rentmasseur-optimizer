#!/usr/bin/env python3
"""
RM-CIC Re-classification with Body Evidence

Loads profile body evidence and re-runs the 5-layer classifier.
"""
import json
import re
from pathlib import Path
from datetime import datetime, timezone
from typing import Dict, List, Any

DATA_DIR = Path(__file__).resolve().parent.parent / "data"
RECEIPTS_DIR = Path(__file__).resolve().parent.parent / "receipts"

# Term definitions (same as rm_cic.py)
CLIENT_TITLE_TERMS = {
    "looking for male massage": 80,
    "looking for massage": 75,
    "looking for therapist": 70,
    "looking for male massage ther": 80,
    "seeking massage": 65,
    "need massage": 60,
    "want massage": 60,
    "want a massage": 60,
    "need a massage": 60,
}

CLIENT_USERNAME_TERMS = {
    "inneed": 35,
    "looking": 30,
    "need": 25,
    "luv": 20,
    "love": 20,
    "wants": 20,
    "sore": 20,
    "aching": 20,
    "relax": 15,
    "body": 10,
}

PROVIDER_TITLE_TERMS = {
    "male masseur": -90,
    "masseur": -80,
    "gay massage in": -75,
    "massage in": -65,
    "bodywork by": -60,
    "therapist in": -55,
    "deep tissue": -50,
    "swedish": -50,
    "sports massage": -50,
    "therapeutic massage": -50,
    "tantric": -45,
}

PROVIDER_USERNAME_TERMS = {
    "masseur": -55,
    "massage": -45,
    "bodywork": -40,
    "hands": -35,
    "touch": -35,
    "healing": -35,
    "spa": -35,
    "therapist": -35,
    "deep": -30,
    "swedish": -30,
    "sportsmassage": -35,
}

CLIENT_BODY_TERMS = {
    "looking for a masseur": 100,
    "looking for massage therapist": 90,
    "looking for a massage therapist": 90,
    "seeking massage": 85,
    "need a massage": 80,
    "want a massage": 70,
    "can host": 35,
    "visiting nyc": 30,
    "available tonight": 30,
}

PROVIDER_BODY_TERMS = {
    "i offer massage": -100,
    "book me": -90,
    "my rates": -85,
    "incall": -80,
    "outcall": -80,
    "deep tissue": -75,
    "swedish massage": -70,
    "sports massage": -70,
    "therapeutic massage": -65,
    "licensed massage therapist": -60,
    "my clients": -60,
    "services": -45,
    "session": -35,
    "availability": -25,
}


def normalize(text: str) -> str:
    text = text or ""
    text = text.lower()
    text = re.sub(r"\s+", " ", text)
    return text.strip()


def apply_terms(text: str, terms: Dict[str, int], prefix: str) -> tuple[int, List[str]]:
    score = 0
    reasons = []
    for term, weight in terms.items():
        if term in text:
            score += weight
            reasons.append(f"{prefix}:{term}:{weight}")
    return score, reasons


def classify_with_body(profile: Dict[str, Any], body_evidence: Dict[str, Any]) -> Dict[str, Any]:
    """Classify with body evidence."""
    username = normalize(profile.get("username", ""))
    title = normalize(profile.get("title", ""))
    
    # Combine bio, services, rates, availability into body text
    body_parts = [
        body_evidence.get("bio", "") or "",
        body_evidence.get("services", "") or "",
        body_evidence.get("rates", "") or "",
        body_evidence.get("availability", "") or "",
        body_evidence.get("location", "") or "",
        body_evidence.get("contact_button", "") or "",
    ]
    body = normalize(" ".join(body_parts))

    score = 0
    reasons: List[str] = []

    # Layer 1: Title
    s, r = apply_terms(title, CLIENT_TITLE_TERMS, "client_title")
    score += s
    reasons.extend(r)

    # Layer 2: Username
    s, r = apply_terms(username, CLIENT_USERNAME_TERMS, "client_username")
    score += s
    reasons.extend(r)

    # Layer 4: Provider title
    s, r = apply_terms(title, PROVIDER_TITLE_TERMS, "provider_title")
    score += s
    reasons.extend(r)

    # Layer 4: Provider username
    s, r = apply_terms(username, PROVIDER_USERNAME_TERMS, "provider_username")
    score += s
    reasons.extend(r)

    # Layer 3: Body (now available)
    if body:
        s, r = apply_terms(body, CLIENT_BODY_TERMS, "client_body")
        score += s
        reasons.extend(r)
        s, r = apply_terms(body, PROVIDER_BODY_TERMS, "provider_body")
        score += s
        reasons.extend(r)

    # Generic title bonus
    if not title or title == "rentmasseur.com" or "| rentmasseur" in title:
        score += 10
        reasons.append("generic_or_blank_title:+10")

    # Classification
    if score >= 70:
        label = "client_likely"
        confidence = min(0.98, 0.70 + (score - 70) / 200)
        next_action = "export_to_A_clients_review"
    elif score >= 35:
        label = "client_possible"
        confidence = min(0.75, 0.45 + (score - 35) / 150)
        next_action = "crawl_profile_body_or_manual_review"
    elif score >= -25:
        label = "unknown"
        confidence = 0.40
        next_action = "low_priority_revisit"
    else:
        label = "provider_likely"
        confidence = min(0.98, 0.70 + abs(score) / 200)
        next_action = "exclude_from_client_outreach"

    return {
        "username": profile.get("username"),
        "url": profile.get("url"),
        "city": profile.get("city"),
        "title": profile.get("title"),
        "client_score": score,
        "label": label,
        "confidence": round(confidence, 2),
        "reasons": reasons,
        "next_action": next_action,
        "has_body_evidence": bool(body),
        "body_length": len(body)
    }


def main():
    print("=== RM-CIC RE-CLASSIFICATION WITH BODY EVIDENCE ===")

    # Load original classification audit
    audit_file = DATA_DIR / "classification_audit.json"
    with open(audit_file) as f:
        audit_data = json.load(f)
    classified = audit_data.get("classified", [])
    print(f"\n[1] Loaded {len(classified)} classified profiles")

    # Load body evidence
    body_file = DATA_DIR / "profile_bodies.json"
    with open(body_file) as f:
        body_data = json.load(f)
    body_evidence_list = body_data.get("evidence", [])
    body_map = {e["username"]: e for e in body_evidence_list}
    print(f"[2] Loaded body evidence for {len(body_map)} profiles")

    # Re-classify with body evidence
    print("\n[3] Re-classifying with body evidence...")
    reclassified = []
    for p in classified:
        username = p["username"]
        body_evidence = body_map.get(username, {})
        result = classify_with_body(p, body_evidence)
        reclassified.append({
            **p,
            "classification": result,
            "body_evidence": body_evidence
        })

    # Bucket
    print("\n[4] Bucketing...")
    buckets = {
        "client_likely": [],
        "client_possible": [],
        "unknown": [],
        "provider_likely": []
    }
    for p in reclassified:
        label = p["classification"]["label"]
        buckets[label].append(p)

    print(f"  A (client_likely): {len(buckets['client_likely'])}")
    print(f"  B (client_possible): {len(buckets['client_possible'])}")
    print(f"  C (unknown): {len(buckets['unknown'])}")
    print(f"  D (provider_likely): {len(buckets['provider_likely'])}")

    # Save updated bucket files
    print("\n[5] Saving updated bucket files...")
    for label, profiles in buckets.items():
        bucket_letter = {"client_likely": "A", "client_possible": "B", "unknown": "C", "provider_likely": "D"}[label]
        output_file = DATA_DIR / f"clients_{bucket_letter}_{label}_v2.json"
        with open(output_file, "w") as f:
            json.dump({
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "count": len(profiles),
                "profiles": profiles
            }, f, indent=2)
        print(f"  Saved: {output_file}")

    # Save updated client candidates
    client_candidates = buckets["client_likely"] + buckets["client_possible"]
    candidates_file = DATA_DIR / "client_candidates_v2.json"
    with open(candidates_file, "w") as f:
        json.dump({
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "count": len(client_candidates),
            "profiles": client_candidates
        }, f, indent=2)
    print(f"  Saved candidates: {candidates_file}")

    # Save updated summary
    summary = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "total_profiles": len(reclassified),
        "client_likely": len(buckets["client_likely"]),
        "client_possible": len(buckets["client_possible"]),
        "unknown": len(buckets["unknown"]),
        "provider_likely": len(buckets["provider_likely"]),
        "client_candidates": len(client_candidates),
        "next_best_action": "expand search to major US cities for 1000+ clients"
    }
    summary_file = DATA_DIR / "client_detection_summary_v2.json"
    with open(summary_file, "w") as f:
        json.dump(summary, f, indent=2)
    print(f"  Saved summary: {summary_file}")

    # Save receipt
    ts = datetime.now(timezone.utc).isoformat().replace(":", "-")
    receipt_file = RECEIPTS_DIR / f"rm_cic_v2_{ts}.json"
    with open(receipt_file, "w") as f:
        json.dump({
            "action": "rm_cic_reclassification_with_body",
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "summary": summary,
            "buckets": {k: len(v) for k, v in buckets.items()}
        }, f, indent=2)
    print(f"  Receipt: {receipt_file}")

    print("\n=== COMPLETE ===")
    print(f"Client candidates (A+B): {len(client_candidates)}")
    print(f"  - A (contact-ready): {len(buckets['client_likely'])}")
    print(f"  - B (needs verification): {len(buckets['client_possible'])}")
    print(f"Next action: Expand search to major US cities for 1000+ clients")


if __name__ == "__main__":
    main()
